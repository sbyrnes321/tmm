...
Written by Steve Byrnes, 2012. Please email me any feedback. My website is \url{http://sjbyrnes.com} . Last update/upload: 29 March 2012.
...
There are three files: \verb=fresnel.py= contains all the main programs, \verb=fresnel_samples.py= contains a few example calculations to get you started, and \verb=fresnel_tests.py= contains a number of programs that perform various tests and consistency checks to confirm that everything is coded and running correctly.
...
Requires SciPy and NumPy to run. The ``examples'' also use Matplotlib.

Version requirements: Personally, I am using Python 2.7.2, SciPy 0.9.0, NumPy 1.6.1, Matplotlib 1.1.0, within Spyder 2.0.13.
...
OK, now that you have successfully installed Python, NumPy, SciPy, etc., you can start by typing, for example,

\begin{verbatim}
In [1]: sys.path.append('C:\\FolderWhereIDownloadedThePrograms')
In [2]: import fresnel_samples
In [3]: fresnel_samples.sample1()
\end{verbatim}
...
At the interface between the $(n-1)$st and $n$th material, let $v_n$ be the amplitude of the wave on the $n$th side heading forwards (away from the boundary), and let $w_n$ be the amplitude on the nth side heading backwards (towards the boundary). (Fig.~\ref{vwdefn}.) (($v_0,w_0$) are undefined, while $v_N=t$ and $w_N=0$.) Then
...
\S \cdot \n &=& \frac12 \Re[ \n \cdot (\E^* \times \H)] \propto \Re[(E_f^*+E_b^*) (E_f - E_b)n \cos \theta]\eea
After normalizing to incident power:
$$\text{s-polarization:} \qquad \S \cdot \n = \frac{\Re\[ (n) (\cos \theta) (E_f^* + E_b^*) (E_f - E_b)\]}{\Re\[ n_0 \cos \theta_0\] }$$

Next, p-polarization:
\bea
\E &=& E_f(-\sin \theta \z +  \cos \theta \x) + E_b(- \sin \theta \z -  \cos \theta \x) \\
\H &\propto& n E_f \y + n E_b \y \\
\S \cdot \n &=& \frac12\Re[ \n \cdot (\E^* \times \H)] \propto \Re[(\cos \theta)^*(E_f^*-E_b^*) (E_f + E_b)n ]\eea
After normalizing to incident power:
$$\text{p-polarization:} \qquad \S \cdot \n = \frac{\Re\[ (n) (\cos \theta^*) (E_f + E_b) (E_f^* - E_b^*)\]}{\Re \[ n_0 \cos \theta_0^* \]}$$
...
$$\text{s-polarization:} \qquad T = |t|^2\frac{ \Re\[ n \cos \theta\]}{\Re \[ n_0 \cos \theta_0 \] }$$
$$\text{p-polarization:} \qquad T = |t|^2\frac{ \Re\[ n \cos \theta^* \]}{\Re \[ n_0 \cos \theta_0^* \] }$$
...
\bea
\text{s-polarization:} \qquad a(z) &=& \frac{|E_f+E_b|^2 \Im\[n \cos(\theta) k_z\]}{\Re\[ n_0 \cos \theta_0 \] } \\
\text{p-polarization:} \qquad a(z) &=& \frac{\Im\[n \cos(\theta^*)\(k_z|E_f-E_b|^2-k_z^*|E_f+E_b|^2\)\]}{\Re \[ n_0 \cos \theta_0^* \] }
\eea
...
where:
\bea
\text{s-polarization}: A_1 &=& \frac{\Im\[n \cos(\theta) k_z\]}{\Re\[ n_0 \cos \theta_0 \] }|w|^2\\
A_2 &=& \frac{\Im\[n \cos(\theta) k_z\]}{\Re \[n_0 \cos \theta_0 \] }|v|^2\\
A_3 &=& \frac{\Im\[n \cos(\theta) k_z\]}{\Re \[n_0 \cos \theta_0 \] }vw^*\\
\text{p-polarization}: A_1 &=& \frac{2\Im\[k_z\] \Re\[n \cos(\theta^*)\]}{\Re \[n_0 \cos \theta_0^* \] } |w|^2\\
A_2 &=& \frac{2\Im\[k_z\] \Re\[n \cos(\theta^*)\]}{\Re\[ n_0 \cos \theta_0^* \] } |v|^2\\
A_3 &=& \frac{2\Re\[k_z\] \Im\[n \cos(\theta^*)\]}{\Re \[ n_0 \cos \theta_0^* \] } vw^*
\eea
where $v = E_f(0)$ and $w=E_b(0)$.

\section{Branch cuts}

Snell's law gives $\theta_i = \arcsin(n_0 \sin(\theta_0)/n_i)$. However, the arcsin function is ambiguous--it has branch cuts in the complex plane. How to get the right $\theta$?? With different choices of $\theta$, you can flip the sign of $\cos \theta$, so it's important to get it right.
...
In more specialized cases, like negative-index materials ($\Re n <0$) or stimulated-emission media ($\Im n<0$), results are not guaranteed! Please check this before using!
...
$$\text{power entering} = \frac{\Re\[ n_0 \cos \theta_0  (1 + r^*) (1 -r)\]}{\Re\[ n_0 \cos \theta_0\] } = (1-R) + 2 \Im[r]\frac{\Im[n_0 \cos \theta_0]}{\Re[n_0 \cos\theta_0]}$$
...
\be{Snoninterfered} \S(z)\cdot \n = e^{\alpha |z|} - R e^{-\alpha |z|} \qquad \text{[formula for without wave  interference]} \quad \ee
...
Therefore,
$$-\pi/2 < -\phi \leq \arg \[ \frac{n_i^*}{n_i} \sqrt{n_i^2 - (n_0 \sin \theta_0)^2}\] < -2\phi+\pi/2 < \pi/2$$
